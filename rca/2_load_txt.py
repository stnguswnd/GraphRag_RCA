...
# 1. 경로 설정
# -------------------------
# 보험 파이프라인의 2_load_pdfplumber.py에 대응하는 파일.
# 입력이 PDF가 아니라 data/rca_mock/docs/ 아래의 .txt 문헌들이다.
# =========================

BASE_DIR = Path(__file__).resolve().parent          # rca/ 폴더
PROJECT_DIR = BASE_DIR.parent                        # 프로젝트 루트

DOCS_DIR = PROJECT_DIR / "data" / "rca_mock" / "docs"
...
def load_txt_documents(docs_dir: Path) -> list[Document]:
    ...
    if not txt_paths:
        raise FileNotFoundError(f".txt 문헌이 없습니다: {docs_dir}")
