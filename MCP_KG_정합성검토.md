...
> - `outputs/hypotheses.json` (schema **v2.4**, 가설 368건) + `KG_output_명세.md`
...
| X5 | **시나리오 체인 라우팅 정보 부족** | `evidence_label`(A3 / A2·A6 / A5 분기)과 `mapping.drift`(T8 힌트)는 제공되나, **A2(일반 정비) vs A6(소모품 마모)를 가를 정보가 없음** — 둘 다 `Maintenance`. **조치**: Maintenance 노드 `consumable` 속성 또는 출력 `scenario_hint` 필드 (→ Q4) |
...
| X2잔여 | **커버리지 공백 (개정판으로 축소)** | Center-세정은 문서 매핑표에서 **취소선 삭제됨** → 해당 공백은 소멸 (KG에 근거 없던 것과 이제 일치). 잔여: `Edge-Ring→CMP`(25%)와 Edge-Ring-세정(15%)·Scratch-브러시(20%)의 문헌 근거는 여전히 빈약 — CLEAN FailureMode 소수. **조치**: CLEAN troubleshooting 레퍼런스 추가 |
...
| X8 | **표기 규약 불일치** | step: 문서는 한국어(`step=증착/세정`), KG·`lot_history.step`은 코드(`DEPO`/`CLEAN`). param: 개정판 시나리오의 `RF_power`(대문자) vs fab `rf_power`. **조치(문서 측)**: 코드·소문자 표기로 통일 |
...
| Q4 | A2/A6 구분 — Maintenance `consumable` 속성 vs 출력 `scenario_hint` 필드 | X5 |
...
4. Q4 `scenario_hint`/`consumable` + X7 `parts_keyword` — Maintenance dedup(P3)과 한 묶음
5. X8 + X6잔여(MCP 문서 측 소폭 보강) / Q7 매핑표 의도 확인 / Q5 VLM 분류기
